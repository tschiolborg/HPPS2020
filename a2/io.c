...
double* read_points(FILE *f, int* n_out, int *d_out) {
  int read; //number of read elements per fread call 
  int32_t n, d;
  assert(f != NULL);
  ...
  if (read != n) { //did not read expected number of elms
    free(data);
    return NULL;
  } else { //only write to ptrs and return data if all elms were read
    *n_out = n;
    *d_out = d;
    return data;
  }
